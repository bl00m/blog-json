...
class JSONHandler(BlogHandler):
    def get(self, blog_id):
        # get the key from the url, make a custom dicionary with keys stated,
        # give values from the page, write to web page
        # if it's blog front, do a dicionary dump for each blog posting
        if blog_id and blog_id != '/':
            post = Post.by_id(int(blog_id.replace('/','')))
            if post:
